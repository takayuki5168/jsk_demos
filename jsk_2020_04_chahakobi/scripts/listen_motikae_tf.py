...
class MergePointClouds:
    ...
    def callback_semantics(self,msg):
        ...
        if split_action[0] == "vision":
            if split_action[1] == "end":
                ...
                self.points = []
            elif split_action[1] == "motikae":
                print "listen motikae tf"
                self.listen_motikae_tf()
            else:
    ...
    def listen_motikae_tf (self):
        (trans,rot) = self.listener.lookupTransform('/r_gripper_tool_frame', '/l_gripper_tool_frame', rospy.Time(0))
        print "trans"
        print trans
        print "rot"
        print rot
        self.motikae_trans = trans
        self.motikae_rot = rot
        rotmat = self.quat2mat(rot)
        trans_v = np.array(trans)
        trans_v = np.reshape(trans,[3,1])
        trafo = np.vstack([np.hstack([rotmat,trans_v]),np.array([0,0,0,1])])
