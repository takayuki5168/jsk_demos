...
def main():
    ...
    ptcloud_sub = message_filters.Subscriber('/pcl_nodelet/ExtractIndices/output', PointCloud2, queue_size=1, buff_size=2**24)
    semantic_sub = message_filters.Subscriber("/semantic_annotation_merge",Header ,queue_size=1,buff_size=2**24)
    ts = message_filters.ApproximateTimeSynchronizer([ptcloud_sub, semantic_sub],queue_size=100,slop=1,allow_headerless=True)
    ts.registerCallback(Merger.callback_all)
...
class MergePointClouds:
    ...
    def transform_points(self,points):
        ...
        output          point_list: List conatinnig the values of each point, transformed into '/r_gripper_tool_frame' KS
        description     transforms the points from '/head_mount_kinect_rgb_optical_frame' into /r_gripper_tool_frame KS
        """
        xyz = []
        rgb = []
        for point in points:
    ...
    def get_transform(self,now):
        """
        input           none
        output          trafo:  transformation matrix that transforms from '/head_mount_kinect_rgb_optical_frame' to '/r_gripper_tool_frame'
        """
        self.listener.waitForTransform('/r_gripper_tool_frame','/head_mount_kinect_rgb_optical_frame',now,rospy.Duration(4.0))
        (trans,rot) = self.listener.lookupTransform('/r_gripper_tool_frame','/head_mount_kinect_rgb_optical_frame',now)
        # (trans,rot) = self.listener.lookupTransform('/r_gripper_tool_frame', '/head_mount_kinect_rgb_optical_frame', rospy.Time(0))
    ...
    def generate_pointcloud(self,points=None):
        ...
        header.frame_id = "/r_gripper_tool_frame"
